# Release 1B — Round 24 Operator Instructions
...
The package includes `release1b_r24_checksums.py` and `release1b_r24_checksums.sha256` (template).

**Step 1 — Generate baseline (immediately after receipt, before any modification):**

```
cd release1b_round24
python3 release1b_r24_checksums.py --generate
```

Prints SHA-256 and byte count for all 8 artifacts, plus the verifier's own integrity.
Record the verifier's own values separately (out-of-band) per the threat model.

**Step 2 — Verify at any time:**

```
python3 release1b_r24_checksums.py --verify
```
...
**Unresolved Provenance Limitation (D23-11/D24-12):**
SHA-256 cannot be pre-computed in the AI delivery environment. This is an irresolvable
delivery constraint. The operator-generated baseline is authoritative.
...
## §3 Archive Verification (GitHub API — PocketBase v0.29.3)
...
Note: PocketBase v0.29.3 uses `/api/collections/_superusers/auth-with-password`.
The legacy `/api/admins/` endpoint is not used in any harness call.

---

## §4 Pre-Run Static Checks

Run these against the final immutable artifacts before starting PocketBase:

```
chmod +x release1b_cp0.zsh
zsh -n release1b_cp0.zsh                          # syntax — must exit 0
node --check release1b_otp_test_adapter.pb.js      # JS syntax — must exit 0
python3 -m py_compile release1b_r24_checksums.py   # Python syntax — must exit 0
...
Also scan for:
```
grep -n "api/admins" release1b_cp0.zsh           # must yield 0 matches
grep -n "pocketbase admin" release1b_cp0.zsh     # must yield 0 matches
grep -n 'cmd += \[.*hdr' release1b_cp0.zsh       # must yield 0 matches (D24-3)
```
...
- [ ] `python3 release1b_r24_checksums.py --verify` exits 0 with 8 OK
...
- [ ] `--harness-check` exits 0 (includes static duplicate-ID check)
...
```
REPORT="${HOME}/r24_cp0_report_$(date +%Y%m%d_%H%M%S).md"
./release1b_cp0.zsh --run --authorize-cp0 --report-dest="${REPORT}"
```

Exit codes: 0=PASS, 1=FAIL (blocking/error/cleanup), 2=INCOMPLETE (deferred/unresolved).
...
(`=== Isolated root: [/tmp/release1b_cp0_XXXXXX] ===`) then validate before removing:
...
If Python exits nonzero, no deletion occurred.
...
## §9 Email Lifecycle Testing (D24-11)

The email change lifecycle test (`T-EMAIL-LIFECYCLE-CHANGE`) is classified UNRESOLVED.

To resolve:
1. Configure PocketBase in the isolated root to use a local SMTP relay (e.g., `smtp4dev`).
2. Set `--smtp*` flags or PocketBase SMTP settings in the isolated environment.
3. After implementing the test, reclassify from UNRESOLVED to RUNNABLE in a future round.

Do NOT test against production SMTP.
