...
#
# Correction matrix: release1b_round17_review.md
# R17 changes from R16:
#   1. pb_pid_exists(): new helper — distinguishes process gone (ESRCH),
#      process exists + signalable, and EPERM
#   2. pb_trap_cleanup(): corrected termination order — SIGTERM then bounded
#      kill -0 poll, then SIGKILL + bounded poll, then bounded wait (reap
#      only), then PID cleared; unbounded wait no longer precedes timeout
#   3. t_concurrency_auth_group(): pre-worker setup validated step-by-step
#      (ID file, record fetch, email extraction, WRONG_PW_FILE, body gen);
#      per-worker isolated directories used
#   4. t_concurrency_otp_send_group(): new — concurrent OTP-send attempts
#      with per-worker isolation; MANDATORY-DEFERRED if Mailhog absent or
#      OTP endpoint returns 404
#   5. t_concurrency_idempotency_post_group(): new — MANDATORY-DEFERRED
#      (requires idempotency-capable hook endpoint, NEEDS-EXTERNAL)
#   6. t_concurrency_group(): updated to call all three concurrency functions
#   7. cp0_run() final result: three states —
#        FAIL (rc=1): any failure/blocking/harness-err/cleanup-failure
#        INCOMPLETE (rc=2): no failures but mandatory gaps (T_DEFERRED or
#                           T_UNRESOLVED > 0); cannot support CP0 auth
#        PASS (rc=0): no failures and no mandatory gaps
#   8. Defect 23 reclassified IMPL-MD: T-CONCURRENCY-AUTH is IMPL; OTP
#      and idempotency scenarios are MANDATORY-DEFERRED (NEEDS-EXTERNAL)
# ============================================================
...
readonly RELEASE1B_SCRIPT_ROUND="17"
...
pb_wipe_secret_file() {
  ...
  # R15/R16/R17 || true inventory #1 (BENIGN-RETAINED):
...
# R17: helper that checks whether a process PID is still present.
# Return values:
#   0 — process exists and is signalable (kill -0 returned 0)
#   1 — process does not exist (ESRCH)
#   2 — process exists but permission denied (EPERM); treat as still running
# For a child process spawned by this script, EPERM should not occur.
pb_pid_exists() {
  local _pid="$1"
  if kill -0 "$_pid" 2>/dev/null; then
    return 0
  fi
  local _err; _err=$(kill -0 "$_pid" 2>&1)
  case "$_err" in
...
pb_trap_cleanup() {
  if (( RELEASE1B_PB_PID > 0 )); then
    ...
    # Step 1: Send SIGTERM; note result but do not halt on failure.
    # For a child we own, SIGTERM should succeed unless it already exited.
    kill "$_stored_pid" 2>/dev/null
    local _sigterm_rc=$?
    if (( _sigterm_rc != 0 )); then
      print "[trap] WARNING: SIGTERM returned ${_sigterm_rc} for PID ${_stored_pid}" >&2
    fi

    # Step 2: Bounded poll — do NOT call unbounded wait before this block.
    # Polls for up to 5s (10 x 0.5s) using pb_pid_exists, which
    # distinguishes ESRCH (gone), signalable (exists), and EPERM.
    local _poll_rc _tries=0
    pb_pid_exists "$_stored_pid"; _poll_rc=$?
    while (( _poll_rc != 1 && _tries < 10 )); do
      if (( _poll_rc == 2 )); then
        # EPERM: process exists but we cannot signal it — this should not
        # happen for a child we own; treat as still running.
        print "[trap] WARNING: EPERM checking PID ${_stored_pid}; assuming running" >&2
        CLEANUP_FAILURE=1
        break
      fi
      sleep 0.5
    ...
    # Step 3: Escalate to SIGKILL if process still present after poll.
    pb_pid_exists "$_stored_pid"; _poll_rc=$?
    if (( _poll_rc != 1 )); then
      print "[trap] WARNING: PocketBase PID ${_stored_pid} alive after \
${_tries}x0.5s — sending SIGKILL" >&2
      kill -9 "$_stored_pid" 2>/dev/null
      CLEANUP_FAILURE=1
      # Brief confirmation poll after SIGKILL (3s max).
      local _k9_tries=0
      pb_pid_exists "$_stored_pid"; _poll_rc=$?
      while (( _poll_rc != 1 && _k9_tries < 6 )); do
        sleep 0.5
        (( _k9_tries++ ))
        pb_pid_exists "$_stored_pid"; _poll_rc=$?
      done
      if (( _poll_rc != 1 )); then
        print "[trap] ERROR: PocketBase PID ${_stored_pid} still present after SIGKILL" >&2
      fi
    fi

    # Step 4: Bounded reap. At this point the process has either exited
    # (confirmed by kill -0 returning nonzero) or CLEANUP_FAILURE=1 is set.
    # wait returns immediately for a zombie and quickly for a confirmed exit;
    # it is safe here because the termination check precedes it.
    wait "$_stored_pid" 2>/dev/null || true
    # R17 || true inventory #2 (BENIGN-RETAINED):
    # wait returns nonzero when the process was killed by a signal.
    # Termination has already been confirmed via pb_pid_exists above;
    # this call only reaps the zombie entry and is bounded in practice.

    # Step 5: Clear stored PID unconditionally.
    # CLEANUP_FAILURE=1 is already set if termination could not be confirmed.
    RELEASE1B_PB_PID=0
  fi
...
t_harness_selftest() {
  ...
  # Verify pb_pid_exists returns correctly for a non-existent PID.
  pb_pid_exists "99999999" 2>/dev/null
  local _pex=$?
  if (( _pex != 1 )); then
    print "[selftest] FAIL: pb_pid_exists nonexistent pid returned ${_pex} (want 1)" >&2
    fail=1
  fi
...
# §36a — T-CONCURRENCY-AUTH
# Concurrent wrong-password authentication attempts.
# Worker isolation: per-worker directory, body, status, and response files.
# Pre-worker setup validated step-by-step (R17 correction).
t_concurrency_auth_group() {
  print "=== Concurrency auth test ==="
  (( HALT_DEPENDENTS )) && { t_skip "T-CONCURRENCY-AUTH" "blocked"; return 0; }

  # Pre-worker step 1: validate ORDINARY_ID_FILE
  local ord_id; ord_id=$(cat "$ORDINARY_ID_FILE" 2>/dev/null)
  if [[ -z "$ord_id" ]]; then
  ...
  # Pre-worker step 2: fetch user record and validate HTTP status
  ...
  # Pre-worker step 3: extract email and validate result
  local ord_email; ord_email=$(python3 "$PBJ_EXTRACT_PY" "$rp" "email" 2>/dev/null)
  local extract_rc=$?
  rm -f "$rp"
  if (( extract_rc != 0 )) || \
  ...
  if [[ -z "$WRONG_PW_FILE" || ! -f "$WRONG_PW_FILE" ]]; then
    t_harness_err "T-CONCURRENCY-AUTH-SETUP-WRONGPW" \
      "WRONG_PW_FILE not set or missing — alias fixture setup may have failed"
    return 1
  fi
  ...
  # Capture worker exit codes in order
  local -a worker_rcs=()
  local pid
  for pid in "${pids[@]}"; do
    wait "$pid" 2>/dev/null
  ...
  # Classify each result: four categories
  local idx=0
  local rf
...
# §36b — T-CONCURRENCY-OTP-SEND
# Concurrent OTP-send requests to test rate-limit consistency.
# MANDATORY-DEFERRED if Mailhog is absent or OTP endpoint returns 404.
# Worker isolation: per-worker directory, body, status, response files.
t_concurrency_otp_send_group() {
  print "=== Concurrency OTP send test ==="
  (( HALT_DEPENDENTS )) && { t_skip "T-CONCURRENCY-OTP-SEND" "blocked"; return 0; }

  if [[ -z "$RELEASE1B_MH_ID" ]]; then
    t_deferred_mandatory "T-CONCURRENCY-OTP-SEND" \
      "Mailhog not running — OTP delivery cannot be verified (NEEDS-EXTERNAL)"
    return 0
  fi
  ...
  local ord_id; ord_id=$(cat "$ORDINARY_ID_FILE" 2>/dev/null)
  if [[ -z "$ord_id" ]]; then
    t_harness_err "T-CONCURRENCY-OTP-SETUP-ID" "ORDINARY_ID_FILE empty or missing"
    return 1
  fi
  ...
  rm -f "$setup_sf" "$setup_rpf"
  if [[ "$rec_status" != "200" ]]; then
    t_harness_err "T-CONCURRENCY-OTP-SETUP-RECORD" \
      "User record retrieval returned ${rec_status}"
    rm -f "$rp"; return 1
  fi
  if [[ ! -f "$rp" ]]; then
    t_harness_err "T-CONCURRENCY-OTP-SETUP-RESPFILE" "Response path file missing"
    return 1
  fi

  # Pre-worker step 3: extract email and validate
  local ord_email; ord_email=$(python3 "$PBJ_EXTRACT_PY" "$rp" "email" 2>/dev/null)
  local extract_rc=$?
  rm -f "$rp"
  if (( extract_rc != 0 )) || \
     [[ -z "$ord_email" || "$ord_email" == "__absent__" || \
        "$ord_email" == ERROR* ]]; then
    t_harness_err "T-CONCURRENCY-OTP-SETUP-EMAIL" \
      "Email extraction failed: '${ord_email}'"
    return 1
  fi

  # Pre-worker step 4: probe OTP endpoint availability
  local probe_body; probe_body=$(pb_secure_tmpfile .json)
  if ! python3 "$PBJ_PY" "$probe_body" "email=${ord_email}" 2>/dev/null; then
    t_harness_err "T-CONCURRENCY-OTP-SETUP-PROBE-BODY" "pbj.py failed"
    rm -f "$probe_body"; return 1
  fi
  local probe_sf; probe_sf=$(pb_secure_tmpfile .http)
  local probe_rpf; probe_rpf=$(pb_secure_tmpfile .rp)
  RELEASE1B_CANONICAL_TMP="$RELEASE1B_TEST_TMP" \
    python3 "$PBJ_HTTP_PY" "$probe_sf" "$probe_rpf" \
      "$(pb_url "/api/collections/users/request-otp")" \
      "" "$probe_body" "POST"
  local probe_status; probe_status=$(cat "$probe_sf" 2>/dev/null)
  local probe_rp; probe_rp=$(cat "$probe_rpf" 2>/dev/null)
  rm -f "$probe_body" "$probe_sf" "$probe_rpf" "$probe_rp"

  if [[ "$probe_status" == "404" ]]; then
    t_deferred_mandatory "T-CONCURRENCY-OTP-SEND" \
      "OTP endpoint returned 404 — OTP may not be enabled on users collection (NEEDS-EXTERNAL)"
    return 0
  fi
  if [[ "$probe_status" != "200" && "$probe_status" != "204" && \
        "$probe_status" != "429" ]]; then
    t_harness_err "T-CONCURRENCY-OTP-SETUP-PROBE" \
      "OTP endpoint probe returned unexpected status: ${probe_status}"
    return 1
  fi

  # Launch N concurrent OTP-send workers with per-worker isolation
  local N=4
  local -a pids=() result_files=() worker_dirs=()
  local fail_count=0 harness_err_count=0
  local i
  for (( i=1; i<=N; i++ )); do
    local wdir; wdir="${RELEASE1B_TEST_TMP}/otp_worker_${RUN_SUFFIX}_${i}"
    if ! { mkdir -p "$wdir" && chmod 700 "$wdir"; }; then
      t_harness_err "T-CONCURRENCY-OTP-WORKER-DIR-${i}" "mkdir failed"
      (( harness_err_count++ ))
      continue
    fi
    worker_dirs+=("$wdir")
    ...
    if ! python3 "$PBJ_PY" "$w_body" "email=${ord_email}" 2>/dev/null; then
      t_harness_err "T-CONCURRENCY-OTP-WORKER-BODY-${i}" "pbj.py failed"
      rm -rf "$wdir"
      worker_dirs=("${worker_dirs[@]:0:${#worker_dirs[@]}-1}")
      (( harness_err_count++ ))
      continue
  ...
  for rf in "${result_files[@]}"; do
    ...
    if (( wrc != 0 )); then
      t_harness_err "T-CONCURRENCY-OTP-WORKER-${idx}" \
        "Worker subshell exited nonzero: rc=${wrc}"
      (( harness_err_count++ ))
      [[ -d "$wdir" ]] && rm -rf "$wdir"
      continue
    ...
    if [[ ! -f "$rf" ]]; then
      t_harness_err "T-CONCURRENCY-OTP-RESULT-MISSING-${idx}" "Result file absent"
      (( harness_err_count++ ))
      [[ -d "$wdir" ]] && rm -rf "$wdir"
      continue
    fi
    ...
    if [[ -z "$s" ]] || ! [[ "$s" =~ ^[0-9]{3}$ ]]; then
      t_harness_err "T-CONCURRENCY-OTP-RESULT-MALFORMED-${idx}" \
        "Malformed status: '${s}'"
      (( harness_err_count++ ))
      continue
    fi

    # Acceptable: 200 (queued), 204 (no content), 429 (rate limited).
    # Anything else is a functional failure.
    if [[ "$s" != "200" && "$s" != "204" && "$s" != "429" ]]; then
      (( fail_count++ ))
    fi
  done

  if (( harness_err_count > 0 )); then
    t_harness_err "T-CONCURRENCY-OTP-SEND" \
      "${harness_err_count}/${N} workers had harness errors"
    return 1
  fi
  ...
  if (( fail_count > 0 )); then
    t_fail "T-CONCURRENCY-OTP-SEND" \
      "${fail_count}/${N} concurrent OTP-send attempts returned unexpected status"
    return 1
  fi
...
# §36c — T-CONCURRENCY-IDEMPOTENCY-POST
# Concurrent POST to an idempotency-enforcing hook endpoint.
# MANDATORY-DEFERRED: the endpoint route, idempotency key semantics, and
# expected duplicate-record behavior are all NEEDS-EXTERNAL.
t_concurrency_idempotency_post_group() {
  ...
  # The idempotency endpoint route, key header name, and expected HTTP
  # behavior for duplicate requests are all NEEDS-EXTERNAL. Per-worker
  # isolation (separate dir, body, auth cfg, response, status file, evidence
  # file per worker) will be implemented when the endpoint is available.
  t_deferred_mandatory "T-CONCURRENCY-IDEMPOTENCY-POST" \
    "Requires idempotency-capable hook endpoint; route, key semantics, and \
duplicate-record behavior are NEEDS-EXTERNAL"
}
...
cp0_run() {
  ...
  # Policy: CONTINUE — optional; email tests gate on RELEASE1B_MH_ID.
  ...
  # Policy: CONTINUE — t_blocking sets HALT_DEPENDENTS per missing collection.
  ...
  if (( HALT_DEPENDENTS )); then
    ...
    # Policy: CONTINUE — t_blocking sets HALT_DEPENDENTS if dir is bad.
  ...
  # Policy: CONTINUE — t_harness_err recorded; tests gate on file existence.
  ...
  # Policy: CONTINUE for all test groups — failures conclusively recorded.
  ...
  # ── R17 three-state final result ─────────────────────────
  #
  # FAIL (rc=1): any functional failure, blocking prerequisite, harness error,
  #              or cleanup failure.
  # INCOMPLETE (rc=2): no failures, but one or more mandatory tests were
  #              deferred (NEEDS-EXTERNAL prerequisite absent) or unresolved.
  #              A INCOMPLETE result cannot support Checkpoint 0 authorization.
  # PASS (rc=0): no failures and no mandatory gaps; all items were reached.
  #
  # T_DEFERRED: mandatory tests skipped due to missing external prerequisite.
  # T_UNRESOLVED: items where NEEDS-EXTERNAL config was not supplied.
  # Both contribute to INCOMPLETE, not FAIL.
  ...
  if (( T_DEFERRED > 0 || T_UNRESOLVED > 0 )); then
    ...
    print "=== ${T_DEFERRED} mandatory-deferred and ${T_UNRESOLVED} unresolved item(s) ==="
    print "=== require NEEDS-EXTERNAL prerequisites before this result can be ==="
    print "=== used to support Checkpoint 0 authorization. ==="
