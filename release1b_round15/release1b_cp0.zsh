...
# ============================================================
#
# AUTHORIZATION BOUNDARY (defect-31, preserved from R14)
# ──────────────────────────────────────────────────────
...
#
# Correction matrix: release1b_round15_review.md
# R15 changes from R14:
#   1. E4/E6/E8 corrected to historical meanings (defect-21, R14 wrong)
#   2. Migration copy/chmod now fail-closed (defect-11, R14 incomplete)
#   3. Legacy fixture deletion now fail-closed (defect-11, R14 incomplete)
#   4. Concurrency wait: || true removed, intent documented
#   5. Archive hash: fail-open || true removed; now blocking
#   6. Defect-16 reclassified IMPL-MD (interceptor baseline NEEDS-EXTERNAL)
#   7. || true inventory: 2 benign retained (dd wipe, kill-wait); 5 eliminated
# ============================================================
...
readonly RELEASE1B_SCRIPT_ROUND="15"
...
# Defect-15: per-hook probe routes (defect-28 enforces key-set match)
...
# Defect-16 (reclassified IMPL-MD in R15):
# 404 = hook-only route; interceptor baseline = UNRESOLVED (NEEDS-EXTERNAL)
...
# Defect-12: __pb_null__ sentinel distinguishes JSON null from ""
...
pb_wipe_secret_file() {
  ...
  # R15 || true inventory #1 (BENIGN-RETAINED):
  ...
  # file regardless of whether the overwrite completed. This is a
  # best-effort wipe; the file is removed either way.
  rm -f "$f"
}
...
pb_trap_cleanup() {
  if (( RELEASE1B_PB_PID > 0 )); then
    ...
    # R15 || true inventory #2 (BENIGN-RETAINED):
    # wait returns non-zero when the process was killed by a signal.
    # This is expected and normal for a process terminated with kill.
    # The non-zero exit code carries no actionable information here.
    RELEASE1B_PB_PID=0
  fi
...
PYEOF
  ...
  # Defect-29/30: sanitized output; atomic write
...
pb_check_package_completeness() {
  ...
  # Defect-28: HOOK_PROBE_ROUTES keys must exactly match HOOK_SRC_PATHS keys
...
t_harness_selftest() {
  ...
  # Defect-1: not reproduced in R13. Single-invocation guard.
...
pb_apply_schema_migrations() {
  # R15 fix: migration copy and chmod are now fail-closed.
  # A glob failure (no .js files) or a cp/chmod error is blocking.
...
pb_create_local_superuser() {
  ...
  # Defect-8: inline id extraction via pbj_extract.py
...
# ────────────────────────────────────────────────────────────
# §14 SCHEMA VERIFICATION (defect-5: after superuser auth)
# ────────────────────────────────────────────────────────────
...
pb_remove_hook_verified() {
  # Defect-16 (IMPL-MD in R15): per-hook HOOK_EXPECTED_REMOVED_STATUS
  local hk="$1"
  local dst="${RELEASE1B_PB_HOOKS_DIR}/${hk}.js"
...
pb_apply_rule_local() {
  ...
  # Defect-12: __pb_null__ sentinel
...
pb_restore_rule_local() {
  # Defect-14: re-read and compare after patch
  ...
  # Defect-14: independent re-read and compare
  sleep 0.2
  url=$(pb_url "/api/collections/${collection}")
  status_f=$(pb_secure_tmpfile .http)
  resp_path_f=$(pb_secure_tmpfile .rp)
...
pb_delete_legacy_fixture() {
  # R15 fix: fail-closed deletion.
  # Each record deletion attempt is tracked independently.
  # If pb_delete_record fails for both collection guesses, CLEANUP_FAILURE=1.
...
pb_setup_alias_group() {
  ...
  # Defect-10: store timing-legacy password for case 5 re-auth
...
t_file_auth_anon_list_rejected() {
  ...
  pb_capture "GET" "/api/collections/progress_notes/records" \
    ...
    local rp; rp=$(cat "$resp_path_f" 2>/dev/null); rm -f "$status_f" "$resp_path_f" "$rp"; return 1
  }
  local rp; rp=$(cat "$resp_path_f" 2>/dev/null); rm -f "$status_f" "$resp_path_f" "$rp"
...
# ────────────────────────────────────────────────────────────
# §26 ALIAS ENUM TESTS (defect-9: cases 1-4 all expect 400)
# ────────────────────────────────────────────────────────────
...
t_concurrency_auth_group() {
  ...
  # R15 fix: || true removed. Worker exit code intentionally not checked.
  # Results are verified via result_files below; wait non-zero is expected
  # for a background subshell that itself exits cleanly (rc=0 from the
  # compound command). In practice wait rc is irrelevant to the test.
  local pid
  for pid in "${pids[@]}"; do
    wait "$pid" 2>/dev/null
    :
...
pb_record_production_exclusions() {
  # R15 correction: exact historical meanings restored.
  # R14 had E4=proxy-IP, E6=push-broadcast, E8=WhatsApp — all wrong.
...
cp0_run() {
  ...
  # Defect-5: verify_schema after superuser auth
...
main() {
  ...
  case "$mode" in
    ...
    --run)
      # Defect-31: Checkpoint 0 is not authorized.
      # Separate explicit --authorize-cp0 flag required.
      local cp0_auth="${2:-}"
      ...
      pb_write_scripts
      pb_detect_platform
      # R15 fix: archive hash failure is now blocking (fail-closed).
